...
class Crowd_DeformableDETR(DetectionTransformer):
    ...
    def __init__(self,
                 *args,
                 ...
                 use_feature_query: bool = True,
                 **kwargs) -> None:
        self.with_box_refine = with_box_refine
        self.as_two_stage = as_two_stage
        self.num_feature_levels = num_feature_levels
        self.query_feature_level =  query_feature_level
        self.use_feature_query = use_feature_query
    ...
    def _init_layers(self) -> None:
        ...
        if not self.as_two_stage and not self.use_feature_query:
    ...
    def points_queris_embed(self, img_feats, batch_img_shape, stride=8, src=None, **kwargs):
        ...
        shape = image_shape // stride # (image_shape + stride//2 -1) // stride
    ...
    def pre_decoder(self, mlvl_feats, batch_data_samples, memory: Tensor, memory_mask: Tensor,
        ...
        if self.as_two_stage:
            ...
            if self.use_feature_query:
                ...
                assert isinstance(query_fea_lvl, list)
                query_list = []
                query_pos_list = []
                points_queries_list = []
                ...
                points_queries = torch.cat(points_queries_list, dim=0)

                # query_fea_lvl = self.query_feature_level
                # stride = 4 * (2 ** query_fea_lvl)
                # query_pos = self.positional_encoding(masks) # (N, C, H, W)
                ...
                reference_points = points_queries.to(query.device).flip(dims=[-1]).unsqueeze(0).repeat(batch_size, 1, 1) # (h, w) -> (w, h)
            else:
                enc_outputs_class, enc_outputs_coord = None, None
