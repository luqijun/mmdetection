...
from ..layers import (DeformableDetrTransformerDecoder,
                      DeformableDetrTransformerEncoder1, SinePositionalEncoding)
from .base_detr import DetectionTransformer


@MODELS.register_module()
class Crowd_DeformableDETR_Encoder(DetectionTransformer):
    ...
    def __init__(self,
                 *args,
                 ...
                 query_feature_level: int = 2,
                 **kwargs) -> None:
        self.with_box_refine = with_box_refine
        self.as_two_stage = as_two_stage
        self.num_feature_levels = num_feature_levels
        self.query_feature_level =  query_feature_level
    ...
    def _init_layers(self) -> None:
        ...
        self.encoder = DeformableDetrTransformerEncoder1(**self.encoder)
    ...
    def forward_transformer(self,
        ...
        if self.training:
            encoder_outputs_dict['enc_outputs_class'] = None
            encoder_outputs_dict['enc_outputs_coord'] = None

        # encoder_outputs_dict['batch_data_samples'] = batch_data_samples

        # encoder_outputs_dict['mlvl_feats'] = img_feats
    ...
    def forward_encoder(self, feat: Tensor, feat_mask: Tensor,
        ...
        """
        memorys, reference_points = self.encoder(
            query=feat,
            query_pos=feat_pos,
        ...
        encoder_outputs_dict = dict(
            ...
            # spatial_shapes=spatial_shapes,
            hidden_states = memorys,
            references=reference_points
        )
        return encoder_outputs_dict


    def points_queris_embed(self, img_feats, batch_img_shape, stride=8, src=None, **kwargs):
        """
        Generate point query embedding during training
        """
        # dense position encoding at every pixel location
        dense_input_embed = img_feats
        bs, c = dense_input_embed.shape[:2]

        # get image shape
        image_shape = torch.tensor(batch_img_shape)
        ...
        points_queries = points_queries.float()
        points_queries[:, 0] /= image_shape[0]
        points_queries[:, 1] /= image_shape[1]

        return query_embed, points_queries, query_feats

    def pre_decoder(self, mlvl_feats, batch_data_samples, memory: Tensor, memory_mask: Tensor,
        ...
        if self.as_two_stage:
            ...
            query_fea_lvl = self.query_feature_level
            if isinstance(query_fea_lvl, list):
                query_list = []
                query_pos_list = []
                for q_f_l in query_fea_lvl:
                    stride = 8 * (2 ** query_fea_lvl)
                    query_pos = self.positional_encoding(masks)  # (N, C, H, W)
                    query_pos, points_queries, query = self.points_queris_embed(query_pos,
                                                                                batch_img_shape=batch_input_shape,
                                                                                stride=stride,
                                                                                src=mlvl_feats[query_fea_lvl])
                    query = query.flatten(2).permute(0, 2, 1)  # (N, H * W, C)
                    query_pos = query_pos.flatten(2).permute(0, 2, 1)  # (N, H * W, C)
                    query_list.append(query)
                    query_pos_list.appen(query_pos)
                query = torch.cat(query_list, dim=1)
                query_pos = torch.cat(query_pos_list, dim=1)
            else:
                stride = 8 * (2 ** query_fea_lvl)
                query_pos = self.positional_encoding(masks) # (N, C, H, W)
                query_pos, points_queries, query = self.points_queris_embed(query_pos, batch_img_shape=batch_input_shape, stride=stride, src=mlvl_feats[query_fea_lvl])
                query = query.flatten(2).permute(0, 2, 1)  # (N, H * W, C)
                query_pos = query_pos.flatten(2).permute(0, 2, 1) # (N, H * W, C)

            # 法2
            # query = mlvl_feats[2] # (N, C, H, W)
            # query = query.flatten(2).permute(0, 2, 1) # (N, C, H, W)
            ...
            reference_points = points_queries.to(query.device).flip(dims=[-1]).repeat(batch_size, 1, 1) # (h, w) -> (w, h)
