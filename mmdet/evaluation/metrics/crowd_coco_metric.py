...
class Crowd_CocoMetric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            result['count'] = len(result['scores'])
            # encode mask to RLE
            if 'masks' in pred:
            ...
            result['MAE'] = abs(result['count'] - gt['count'])
            result['MSE'] = (result['count'] - gt['count']) *  (result['count'] - gt['count'])
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            if metric == 'mae' or metric == 'bbox':
                mae = sum([pre['MAE'] for pre in preds]) / len(preds)
                mse = np.sqrt(sum([pre['MSE'] for pre in preds]) / len(preds))
